...
contract OrbitrumStable is ERC20, L2Gateway {
  ...
  mapping(address => mapping(address => uint256)) private userDeposit;
  ...
  function stableDeposit(IERC20 _token, uint256 _amount) public {
    ...
    userDeposit[msg.sender][address(_token)] += _amount;
  ...
  function stableWithdraw(IERC20 _token, uint256 _amount) public {
    ...
    if (msg.sender != _l2Gateway) {
      require(userDeposit[msg.sender][address(_token)] >= _amount, 'NOT Balances');
    }
    // swap token for Orbitrum stable
