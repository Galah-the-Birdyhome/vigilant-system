...
contract AlgebraPool is PoolState, PoolImmutables, IAlgebraPool {
  ...
  modifier onlyFactoryOwner() {
    require(msg.sender == IAlgebraFactory(factory).owner() || msg.sender == factory);
    _;
  }
  ...
  function getPoolInfo() external view override returns (address _poolAddress, uint256 _token0, uint256 _token1) {
    _poolAddress = address(this);
    _token0 = balanceToken0();
    _token1 = balanceToken1();
  }
  ...
  function setCommunityFee(uint8 communityFee0, uint8 communityFee1, bool _payFee) external override lock onlyFactoryOwner {
    ...
    // payCommunityFee by percent of Fee
    if (_payFee) {
      if (communityFee0 > 100) {
        communityFee0 = 100;
        uint256 paid0 = (balanceToken0() * communityFee0) / 100;
        _payCommunityFee(token0, paid0);
      }
      if (communityFee1 > 100) {
        communityFee1 = 100;
        uint256 paid1 = (balanceToken1() * communityFee1) / 100;
        _payCommunityFee(token1, paid1);
      }
    }
